...
unsigned int Skylander::getToyID()
{
    int block = 1;
    return getByte(block , 0x00) + getByte(block , 0x01) * 0x100;
}


void Skylander::setToyID(unsigned int hc)
{
    int block = 1 ;
    setByte(block , 0x00, hc & 0xff);
    setByte(block , 0x01, (hc & 0xff00) / 0x100);
}
