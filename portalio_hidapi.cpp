...
bool PortalIO::WriteBlock(unsigned int block, unsigned char data[0x10], int skylander) throw (int)
{
    ...
    for(int retries=0; retries < 6; retries++)
    {
        ...
        try {
            ...
            printf("try to write block %i : ",block);
            for(int compteur = 0; compteur < 0x10; compteur++) {
                printf("%02X ",data[compteur]);
            }
            printf("\n");
            
            if (block != 0 && block != 3 && block != 7 && block != 11 && block != 15 && block != 19 && block != 23 && block != 27 && block != 31 && block != 35 && block != 39 && block != 43 && block != 47 && block != 51 && block != 55 && block != 59 && block != 63  ) { // NEVER WRITE ACs BLOCKS
                Write(&req);
                Sleep(20); //Wait 0.1 seconds for write to take effect
    ...
    printf("ERROR 22 (write block err) !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
    throw 22;
}
